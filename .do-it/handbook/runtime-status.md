...
3. **Aurora 情绪系统纯本地分析** — 基于关键词/时间/行为的规则引擎，未使用 AI 分析对话情感。
...
- **AI 记忆系统**：
  ...
  - 离线回退到关键词 LIKE 检索
- **Aurora 情绪系统**：
  - 6 种情绪（default/happy/worried/excited/angry/tender）+ 4 级亲密度
  - 基于用户输入关键词、时间上下文、打卡 streak 的行为分析引擎
  - 情绪状态自动注入 AI 聊天 system prompt
  - 前端实时显示情绪 + 亲密度等级
